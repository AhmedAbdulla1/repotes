import 'package:dartz/dartz.dart';
import 'package:reports/app/di.dart';
import 'package:reports/data/network/failure.dart';
import 'package:reports/domain/repository/repository.dart';
import 'package:reports/domain/usecase/base_usecase.dart';


class UpdateStatusUsecase implements BaseUseCase<UpdateStatusInput, String> {
  final Repository _repository = instance<Repository>();

  @override
  Future<Either<Failure, String>> execute(UpdateStatusInput input) async {
    return await _repository.updateStatus(input.path,input.status);
  }
}

class UpdateStatusInput {
  String path;
  String status;
  UpdateStatusInput({
    required this.path,
    required this.status,
});

}
